...
FAKE_VALUE_FUNC(enum CanCommunicationReturnCode,
                fake_send,
                const struct CanCommunicationFrame *);
...
static enum CanCommunicationReturnCode capturing_send(
...
static enum CanCommunicationReturnCode capturing_receive(
    const struct CanCommunicationFrame *frame) {
...
void setUp(void) {
    ...
    captured_send_count = 0U;
    captured_receive_count = 0U;
...
/*!
 * \defgroup can_communication_api_init
 * Tests for can_communication_api_init
 * \{
 */
...
void test_init_success(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(
        ...
        rc,
        "Expected init to succeed on a valid configs array");
...
void test_init_null_configs(void) {
    TEST_ASSERT_EQUAL(
        ...
        can_communication_api_init(NULL));
}
...
void test_init_accepts_null_critical_section_callbacks(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(
        CAN_COMMUNICATION_RC_OK,
        can_communication_api_init(configs),
        "Critical-section callbacks are optional; NULL on both is allowed");
}
...
void test_init_resets_state_on_repeated_call(void) {
    ...
    enum CanCommunicationReturnCode second =
        can_communication_api_init(configs);
    ...
    TEST_ASSERT_EQUAL_MESSAGE(
        CAN_COMMUNICATION_RC_OK,
        second,
        "Re-init must succeed and reset state");
}

/*! \} */

/*!
 * \defgroup can_communication_api_add_to_tx
 * Tests for can_communication_api_add_to_tx
 * \{
 */
...
void test_add_to_tx_invalid_length(void) {
    ...
    frame.length =
        (uint8_t)(CAN_COMMUNICATION_FRAME_DATA_SIZE + 1U);
...
void test_add_to_tx_queue_full(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(
        CAN_COMMUNICATION_RC_QUEUE_FULL,
        rc,
        "Push past capacity must return QUEUE_FULL");

    TEST_ASSERT_TRUE_MESSAGE(
        accepted > 0U,
        "At least one frame must have been accepted before saturation");
}

/*! \} */

/*!
 * \defgroup can_communication_api_add_to_rx
 * Tests for can_communication_api_add_to_rx
 * \{
 */
...
/*!
 * \defgroup can_communications_api_process_tx
 * Tests for can_communications_api_process_tx
 * \{
 */
...
void test_process_tx_empty_queue_succeeds_without_calling_send(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(
        CAN_COMMUNICATION_RC_OK,
        rc,
        "Empty drain must return OK");

    TEST_ASSERT_EQUAL_MESSAGE(
        0,
        fake_send_fake.call_count,
        "Send must not be called when the queue was empty");
}
...
void test_process_tx_drains_a_single_frame(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(
        CAN_COMMUNICATION_RC_OK,
        rc,
        "Drain must return OK when send succeeded");

    TEST_ASSERT_EQUAL_MESSAGE(
        1,
        fake_send_fake.call_count,
        "Send must be called once per queued frame");
}
...
void test_process_tx_drains_every_queued_frame_in_one_call(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(
        5U,
        captured_send_count,
        "Every queued frame must be sent in one process_tx call");
}
...
void test_process_tx_preserves_frame_contents_and_order(void) {
    ...
    struct CanCommunicationFrame first =
        make_frame(0x111U, 3U);

    struct CanCommunicationFrame second =
    ...
    TEST_ASSERT_EQUAL_MESSAGE(
        2U,
        captured_send_count,
        "Both queued frames must be delivered");

    TEST_ASSERT_EQUAL_MESSAGE(
        0x111U,
        captured_send[0].id,
        "FIFO order: first enqueued must be first sent");

    TEST_ASSERT_EQUAL_MESSAGE(
        3U,
        captured_send[0].length,
        "First frame length must be preserved");

    TEST_ASSERT_EQUAL_HEX8_ARRAY_MESSAGE(
        first.data,
        captured_send[0].data,
        CAN_COMMUNICATION_FRAME_DATA_SIZE,
        "First frame data must be preserved");

    TEST_ASSERT_EQUAL_MESSAGE(
        0x222U,
        captured_send[1].id,
        "Second enqueued must be second sent");

    TEST_ASSERT_EQUAL_MESSAGE(
        8U,
        captured_send[1].length,
        "Second frame length must be preserved");

    TEST_ASSERT_EQUAL_HEX8_ARRAY_MESSAGE(
        second.data,
        captured_send[1].data,
        CAN_COMMUNICATION_FRAME_DATA_SIZE,
        "Second frame data must be preserved");
}
...
void test_process_tx_surfaces_send_failure_but_keeps_draining(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(
        CAN_COMMUNICATION_RC_TRANSMISSION_ERROR,
        rc,
        "Failed send must surface as TRANSMISSION_ERROR");

    TEST_ASSERT_EQUAL_MESSAGE(
        3,
        fake_send_fake.call_count,
        "Drain must continue even if individual sends fail");
}

/*! \} */

/*!
 * \defgroup can_communication_api_process_rx
 * Tests for can_communication_api_process_rx
 * \{
 */
...
void test_process_rx_empty_queue_succeeds_without_dispatching(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(
        0,
        fake_on_receive_fake.call_count,
        "on_receive must not be called when the queue was empty");
}
...
void test_process_rx_drains_a_single_frame(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(
        CAN_COMMUNICATION_RC_OK,
        rc,
        "Drain must return OK when dispatcher succeeded");

    TEST_ASSERT_EQUAL_MESSAGE(
        1,
        fake_on_receive_fake.call_count,
        "Dispatcher must be called once per queued frame");
}
...
void test_process_rx_drains_every_queued_frame_in_one_call(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(
        7U,
        captured_receive_count,
        "Every queued frame must be dispatched in one process_rx call");
}
...
void test_process_rx_preserves_frame_contents_and_order(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(
        2U,
        captured_receive_count,
        "Both queued frames must be dispatched");

    TEST_ASSERT_EQUAL_MESSAGE(
        0x711U,
        captured_receive[0].id,
        "FIFO order: first enqueued must be first dispatched");

    TEST_ASSERT_EQUAL_MESSAGE(
        2U,
        captured_receive[0].length,
        "First frame length must be preserved");

    TEST_ASSERT_EQUAL_HEX8_ARRAY_MESSAGE(
        first.data,
        captured_receive[0].data,
        CAN_COMMUNICATION_FRAME_DATA_SIZE,
        "First frame data must be preserved");

    TEST_ASSERT_EQUAL_MESSAGE(
        0x722U,
        captured_receive[1].id,
        "Second enqueued must be second dispatched");

    TEST_ASSERT_EQUAL_MESSAGE(
        5U,
        captured_receive[1].length,
        "Second frame length must be preserved");

    TEST_ASSERT_EQUAL_HEX8_ARRAY_MESSAGE(
        second.data,
        captured_receive[1].data,
        CAN_COMMUNICATION_FRAME_DATA_SIZE,
        "Second frame data must be preserved");
}
...
void test_process_rx_surfaces_dispatcher_failure_but_keeps_draining(void) {
    ...
    TEST_ASSERT_EQUAL_MESSAGE(
        CAN_COMMUNICATION_RC_RECEIVE_HANDLER_ERROR,
        rc,
        "Failed dispatch must surface as RECEIVE_HANDLER_ERROR");

    TEST_ASSERT_EQUAL_MESSAGE(
        3,
        fake_on_receive_fake.call_count,
        "Drain must continue even if individual dispatches fail");
}
