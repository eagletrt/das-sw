/************************************************************************************/ /**
* \file         Source/ARMCM4_STM32G4/can.c
...
static blt_bool CanCalculateBitTimingConfig(blt_int32u const baud,
     ...
     */
        for (prescaler = periph_params->prescaler_min;
             prescaler <= periph_params->prescaler_max;
             prescaler++) {
...
**            it is PCLK1. Alternatives are HSE or PLLQ. To meet the clock tolerance
...
void CanInit(void) {
   ...
   */
    ...
    if (canFdBitrateSwitchUsed == BLT_TRUE) {
        ...
        if (CanCalculateBitTimingConfig(BOOT_COM_CAN_FD_BRS_BAUDRATE, &periphParams, &bittimingConfigBRS) == BLT_FALSE) {
    ...
    if (CanCalculateBitTimingConfig(BOOT_COM_CAN_BAUDRATE, &periphParams, &bittimingConfig) == BLT_FALSE) {
        /* Incorrect configuration. The specified baudrate is not supported for the given
     * clock configuration. Verify the following settings in blt_conf.h:
   ...
   */
    ...
    HAL_FDCAN_ConfigGlobalFilter(&canHandle, FDCAN_REJECT, FDCAN_REJECT, FDCAN_REJECT_REMOTE, FDCAN_REJECT_REMOTE);
     ...
     * - filter: 0
     */
        (void)HAL_FDCAN_ConfigTxDelayCompensation(&canHandle, bittimingConfigBRS.tseg1 * bittimingConfigBRS.prescaler, 0U);
    }
#endif
...
#if (BOOT_COM_CAN_FD_ENABLE > 0)
    static const blt_int8u dlc2len[] = { 0, 1, 2, 3, 4, 5, 6, 7, 8, 12, 16, 20, 24, 32, 48, 64 };
#endif
